...
const OUT_DIR  = path.join(__dirname, '_preview_icons_illustration');

function buildPrompt(food) {
  return `hyperrealistic digital illustration of a single ${food.enName}, highly detailed, glossy, vibrant colors, isolated on pure white background, no plate, no bowl, no table, no environment, centered single subject, icon style, clean edges`;
}

const NEGATIVE = 'photograph, photo, photography, plate, bowl, dish, table, tablecloth, background scene, environment, multiple items, group, cluttered, shadow on ground, realistic photo lighting, studio photo setup, text, watermark, cartoon, chibi, face, eyes, character, person, hands';

const FOODS = [
  { id: 'food_fruit_apple',    name: '苹果',   enName: 'red apple with leaf',      category: 'fruit' },
  { id: 'food_meat_chicken',   name: '鸡肉',   enName: 'raw chicken breast fillet', category: 'meat' },
  { id: 'food_veg_broccoli',   name: '西兰花', enName: 'broccoli floret',           category: 'vegetable' },
];
...
// 关键:去白底 + 缩放到 120x120,保留透明通道,不加背景块
